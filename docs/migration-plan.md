...
### Phase 4: データ移行
0. **Supabase の records / sessions テーブルからテストデータを削除してからエクスポートする**（今日 workoutlog2 でも記録しているため、日付で区別できない）
...
- [x] 3-B-1. getHistory + updateSession/deleteSession（session_id ベース）
- [x] 3-B-2. getExercisesWithLastDate（loadHistExList / loadAnalysisExList）
- [x] 3-B-3. getExerciseHistory（loadHistExDetail / loadS3Hist）
- [x] 3-B-4. getInjuryHistory（loadInjuryHistory / loadS3Injury）
- [x] 3-B-5. getAnalysisData
- [x] 3-B-6. updateExerciseRecords（saveRecordModal / deleteExerciseRecordsConfirm）
- [x] 3-C-1. 設定系 CRUD（種目）
- [x] 3-C-2. 設定系 CRUD（メニュー）
- [x] 3-C-3. 設定系 CRUD（怪我部位）
